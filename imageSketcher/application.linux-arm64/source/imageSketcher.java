...
public void anamorphImage(String filePath) {
  ...
  V = new PVector(img.width*1.15f, 0, img.height*2.55f);
...
public void draw() {
  ...
  if (loading) {
    fill(255);
    rect(width/2-xScale*1.5f, height/2, xScale*3, xScale);
    image(loadingImage, width/2-xScale*1.5f, height/2, xScale*3, xScale);
  }
}
...
public float findMinY(int w, int h) {
...
public float findMaxY(int w, int h) {
  float max = calcAtPoint(0, 0).y;
  for (int x=0; x<w; x++) {
    for (int y=0; y<h; y++) {
      if (calcAtPoint(x, y).y>max) {
...
public float findMinX(int w, int h) {
...
public float findMaxX(int w, int h) {
  float max = calcAtPoint(0, 0).x;
  for (int x=0; x<w; x++) {
    for (int y=0; y<h; y++) {
      if (calcAtPoint(x, y).x<max) {
...
public PVector L(float t) {
  return PVector.add(P, PVector.mult(PVector.sub(V, P), t));
}

public PVector R(float t) {
  return PVector.add(I, PVector.mult(ref, t));
} 

public PVector calcAtPoint(float i, float j) {
